...
class FeishuConfig(BaseModel):
    ...
    enabled: bool = False
    app_id: str = ""  # App ID from Feishu/Lark Open Platform
    app_secret: str = ""  # App Secret from Feishu/Lark Open Platform
    encrypt_key: str = ""  # Encrypt Key for event subscription
    verification_token: str = ""  # Verification Token for event subscription
    domain: str = "feishu"  # "feishu" (open.feishu.cn) or "lark" (open.larksuite.com)
...
class WebSearchConfig(BaseModel):
    ...
    openai_model: str = "gpt-4o-mini"  # Model for OpenAI web search (cheap & fast default)
...
class RiskLimitsConfig(BaseModel):
    """Trading risk limits."""
    max_leverage: int = 10
    max_position_pct: float = 20.0  # Max single position as % of total assets
    require_stop_loss: bool = True


class TradingConfig(BaseModel):
    """Trading configuration (exchange connections, risk limits, data sources)."""
    exchanges_config_path: str = "config/exchanges.yaml"  # Relative to workspace
    default_exchange: str = "binance"
    watched_symbols: list[str] = Field(default_factory=lambda: ["BTC/USDT", "ETH/USDT", "SOL/USDT"])
    risk_limits: RiskLimitsConfig = Field(default_factory=RiskLimitsConfig)

    # Debug: print WS push to console when no real channel is available
    ...
    followin_lang: str = "zh-Hans"

    # Cron intervals (seconds)
    position_scan_interval: int = 900
    anomaly_scan_interval: int = 900
    news_feed_interval: int = 900
    daily_review_hour: int = 22


class Config(BaseSettings):
    """Root configuration for GetAll."""
